...
fun InvoiceAnalysisScreen(onBack: () -> Unit) {
    ...
    Column(
        ...
        if (invoices.isNotEmpty()) {
                ...
                ElevatedCard(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                        ...
                        if (item.rawText.isNotBlank()) {
                            Text(
                                "Texto lido: ${item.rawText.take(220).replace('\n',' ')}${if (item.rawText.length > 220) "..." else ""}",
            ...
            Button(
                onClick = {
                    analysis = analyzeInvoices(
                        invoices.toList(), transformerKva, targetFp, freeMarket, aclPrice,
                        hasGenerator, hasPv, pvKw
                    )
                },
                modifier = Modifier.fillMaxWidth()
            ) { Text("ANALISAR FATURAS") }
        }
...
private fun analyzeInvoices(
    ...
    return buildList {
        ...
        transformerKva.toPtDoubleOrNull()?.let { kva ->
            if (maxDemand != null && kva > 0) add("Relação demanda máxima / potência de transformadores: ${fmt(maxDemand / kva * 100.0, 1)}% (comparação kW/kVA apenas indicativa).")
        }
...
private fun parseInvoice(fileName: String, text: String): InvoiceData {
    ...
    val demand = first(
        listOf(Regex("""(?i)(?:demanda\s+(?:medida|faturada|registrada)|demanda)[^\n]{0,45}?(\d[\d.]*[,]?\d*)\s*kw"""))
    )
    val fp = first(
        listOf(Regex("""(?i)(?:fator\s+de\s+pot[eê]ncia|fp)\s*[:-]?\s*(0[,.]\d{2,4}|1[,.]?0*)"""))
    )
    val reactive = first(
        listOf(Regex("""(?i)(?:energia\s+reativa|reativ[ao])[^\n]{0,45}?(\d[\d.]*[,]?\d*)\s*kvarh"""))
    )
    val total = first(
        listOf(
